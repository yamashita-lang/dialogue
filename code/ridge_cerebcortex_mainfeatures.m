function ridge_cerebcortex_mainfeatures(ID)
% ridge_cerebcortex_mainfeatures.m
% 
% Ridge regression using LLM production and comprehension
%
...
for modality = [1 2]
    for mm = PRM.Features_index
        ii = ii + 1;
        if ii == main_idx || ii == length(PRM.Features)+main_idx
            ...
            ; % skipping low-level features
        end        
    end
end
disp(['features used: ' Features_used ]);
...
for ses = 1:IND.ses_num
    ...
    if idx 
        ...
        MRIwrite(V,[ PRM.SaveDir IND.file_prefix '_ses-' num2str(ses) '_' postfix '.nii']);
    end
